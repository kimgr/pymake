...
import logging, re, os, sys, subprocess, time, traceback
...
class _CommandWrapper(object):
    ...
    def __call__(self, cb):
        self.usercb = cb
        call(self.cline, loc=self.loc, cb=self._cb, context=self.context, **self.kwargs)
...
class _NativeWrapper(_CommandWrapper):
    ...
    def __call__(self, cb):
        ...
        cline_list = parts[2:]
        self.usercb = cb
        call_native(module, method, cline_list,
...
_exec_log = logging.getLogger('pymake.execution')
...
class _MakeContext(object):
    ...
    def remakecb(self, remade, error=None):
        ...
        if remade:
            if self.restarts > 0:
                _exec_log.info("make.py[%i]: Restarting makefile parsing", self.makelevel)
        ...
        if len(self.targets) == 0:
            ...
            _exec_log.info("Making default target %s", self.makefile.defaulttarget)
...
def main(args, env, cwd, cb):
    ...
    try:
        ...
        ostmts, targets, overrides = parsecommandlineargs(arguments)
